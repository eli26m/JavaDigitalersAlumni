...
-- 3.5.5
SELECT ROUND(AVG(CARGO), 2) AS 'PRECIO PROMEDIO', MIN(CARGO) AS 'PRECIO INFERIOR' FROM PEDIDOS_NEPTUNO;
-- 3.5.6
SELECT ROUND(AVG(CARGO), 2) AS 'PRECIO PROMEDIO', MIN(CARGO) AS 'PRECIO INFERIOR', MAX(CARGO) AS 'PRECIO MAXIMO' FROM PEDIDOS_NEPTUNO;

-- 3.5.7
SELECT MAX(PRECIOUNIDAD) AS 'PRECIO MAXIMO', NOMBRECATEGORIA AS CATEGORIA FROM PRODUCTOS_NEPTUNO GROUP BY CATEGORIA;

-- 3.5.8
SELECT COUNT(*) AS ENTREGAS FROM PEDIDOS_NEPTUNO GROUP BY TRANSPORTISTA;

-- 3.5.9
SELECT COUNT(*) AS NACIMIENTOS FROM NACIMIENTOS GROUP BY SEXO;

-- 3.5.10
SELECT ROUND(SUM(CARGO), 2) AS 'TOTAL GASTOS', NOMBRECOMPANIA AS CLIENTE FROM PEDIDOS_NEPTUNO GROUP BY NOMBRECOMPANIA;

-- 3.5.11
SELECT COUNT(*) AS CANTIDAD FROM PRODUCTOS GROUP BY SECCION ORDER BY CANTIDAD DESC;

-- 3.5.12
SELECT * FROM PEDIDOS_NEPTUNO GROUP BY YEAR(FECHAPEDIDO), MONTH(FECHAPEDIDO);
